...
(function initHero3D() {
  ...
  const wireMat = new THREE.LineBasicMaterial({ color: 0xc8f135, transparent: true, opacity: 0.55 });
  ...
  for (let i = 0; i < pos.count; i++) {
    ...
    if (!seen3d.has(k)) { seen3d.add(k); verts.push(pos.getX(i), pos.getY(i), pos.getZ(i)); }
  }
  const dotGeo = new THREE.BufferGeometry();
  dotGeo.setAttribute('position', new THREE.BufferAttribute(new Float32Array(verts), 3));
  const dotMat = new THREE.PointsMaterial({ color: 0xc8f135, size: 0.07, sizeAttenuation: true });
  mesh.add(new THREE.Points(dotGeo, dotMat));

  // ── Anillos orbitales ────────────────────────────────────────
  const ringMat1 = new THREE.MeshBasicMaterial({ color: 0xc8f135, transparent: true, opacity: 0.28 });
  const ring = new THREE.Mesh(new THREE.TorusGeometry(2.05, 0.008, 8, 100), ringMat1);
  ring.rotation.x = Math.PI / 2 + 0.28;
  ring.rotation.z = 0.15;
  scene.add(ring);

  const ringMat2 = new THREE.MeshBasicMaterial({ color: 0xc8f135, transparent: true, opacity: 0.14 });
  const ring2 = new THREE.Mesh(new THREE.TorusGeometry(1.72, 0.004, 8, 80), ringMat2);
  ...
  // ── Satélites ────────────────────────────────────────────────
  const satConfig = [
    { r: 2.6, speed: 0.31, phase: 0,   size: 0.045, ty: 0.6,  tz: 0.2  },
    { r: 2.2, speed: 0.19, phase: 2.1, size: 0.032, ty: -0.4, tz: 1.1  },
    { r: 3.1, speed: 0.13, phase: 4.4, size: 0.025, ty: 1.0,  tz: -0.3 },
    { r: 1.9, speed: 0.47, phase: 1.2, size: 0.018, ty: -0.8, tz: 0.7  },
    { r: 2.8, speed: 0.22, phase: 3.3, size: 0.028, ty: 0.3,  tz: -0.9 },
  ];
  const satellites = satConfig.map(d => {
    const satMat = new THREE.MeshBasicMaterial({ color: 0xc8f135, transparent: true, opacity: 0.75 });
    const sat = new THREE.Mesh(new THREE.SphereGeometry(d.size, 6, 6), satMat);
    scene.add(sat);
    return { mesh: sat, mat: satMat, ...d };
  });
  ...
  for (let i = 0; i < wp.count; i += 2) {
    edgePairs.push([
      new THREE.Vector3(wp.getX(i),   wp.getY(i),   wp.getZ(i)),
      new THREE.Vector3(wp.getX(i+1), wp.getY(i+1), wp.getZ(i+1)),
    ]);
  }
  const pulseMat = new THREE.MeshBasicMaterial({ color: 0xffffff, transparent: true, opacity: 0.9 });
  const pulseMesh = new THREE.Mesh(new THREE.SphereGeometry(0.04, 8, 8), pulseMat);
  mesh.add(pulseMesh);
  let pulseEdge = 0, pulseT3 = 0;

  // ── Luces ────────────────────────────────────────────────────
  scene.add(new THREE.AmbientLight(0x111111, 1));
  const accentLight = new THREE.PointLight(0xc8f135, 1.8, 8);
  accentLight.position.set(2, 2, 3);
  scene.add(accentLight);
  const innerLight = new THREE.PointLight(0xc8f135, 0.9, 4);
  scene.add(innerLight);

  // ── Partículas de fondo ──────────────────────────────────────
  const bgArr = new Float32Array(140 * 3);
  for (let i = 0; i < 140; i++) {
    bgArr[i*3]   = (Math.random() - 0.5) * 12;
    bgArr[i*3+1] = (Math.random() - 0.5) * 10;
    bgArr[i*3+2] = (Math.random() - 0.5) * 6 - 2;
  }
  const bgGeo3d = new THREE.BufferGeometry();
  bgGeo3d.setAttribute('position', new THREE.BufferAttribute(bgArr, 3));
  const bgMatDots = new THREE.PointsMaterial({ color: 0xc8f135, size: 0.022, transparent: true, opacity: 0.25 });
  const bgDots = new THREE.Points(bgGeo3d, bgMatDots);
  scene.add(bgDots);

  // ── Todos los materiales con color acento (para theme toggle) ─
  const accentMaterials = [wireMat, dotMat, ringMat1, ringMat2, bgMatDots];
  satellites.forEach(s => accentMaterials.push(s.mat));

  document.getElementById('btn-tema').addEventListener('click', () => {
    const hex = getAccentHex();
    accentMaterials.forEach(m => m.color.setHex(hex));
    accentLight.color.setHex(hex);
    innerLight.color.setHex(hex);
  });
  ...
  function tick3d() {
    ...
    mesh.rotation.y += 0.004 + mx3 * 0.003;
    mesh.rotation.x += 0.002 - my3 * 0.002;

    const breathe = 1 + Math.sin(t3d * 0.7) * 0.015;
    mesh.scale.setScalar(breathe);

    innerLight.position.set(
      Math.sin(t3d * 0.4) * 0.8,
      Math.cos(t3d * 0.3) * 0.6,
      Math.cos(t3d * 0.5) * 0.7
    );
    ...
    satellites.forEach(s => {
      const a = t3d * s.speed + s.phase;
      s.mesh.position.set(
        Math.cos(a) * s.r,
        Math.sin(a * 0.6 + s.phase) * s.r * 0.3 + s.ty * 0.4,
        Math.sin(a) * s.r * 0.65 + s.tz * 0.2
      );
      s.mat.opacity = (0.6 + Math.sin(t3d * 2 + s.phase) * 0.4) * 0.75;
    });

    pulseT3 += 0.028;
    if (pulseT3 >= 1) { pulseT3 = 0; pulseEdge = (pulseEdge + 1) % edgePairs.length; }
    const [ea, eb] = edgePairs[pulseEdge];
    pulseMesh.position.lerpVectors(ea, eb, pulseT3);
    pulseMat.opacity = 0.9 - Math.abs(pulseT3 - 0.5) * 0.6;

    wireMat.opacity = 0.45 + Math.sin(t3d * 0.5) * 0.1;
    bgDots.rotation.y += 0.0005;
